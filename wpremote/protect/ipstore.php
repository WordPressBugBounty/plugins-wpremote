...
class WPRProtectIpstore_V648 {
	...
	function __construct($storage_type = WPRProtectIpstore_V648::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V648::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V648();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V648();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V648::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V648::STORAGE_TYPE_DB) {
