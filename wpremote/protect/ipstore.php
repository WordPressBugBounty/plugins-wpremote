...
class WPRProtectIpstore_V644 {
	...
	function __construct($storage_type = WPRProtectIpstore_V644::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V644::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V644();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V644();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V644::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V644::STORAGE_TYPE_DB) {
