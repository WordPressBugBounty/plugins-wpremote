...
class WPRProtectIpstore_V588 {
	...
	function __construct($storage_type = WPRProtectIpstore_V588::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V588::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V588();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V588();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V588::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V588::STORAGE_TYPE_DB) {
