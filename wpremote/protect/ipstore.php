...
class WPRProtectIpstore_V577 {
	...
	function __construct($storage_type = WPRProtectIpstore_V577::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V577::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V577();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V577();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V577::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V577::STORAGE_TYPE_DB) {
