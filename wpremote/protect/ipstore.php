...
class WPRProtectIpstore_V639 {
	...
	function __construct($storage_type = WPRProtectIpstore_V639::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V639::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V639();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V639();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V639::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V639::STORAGE_TYPE_DB) {
