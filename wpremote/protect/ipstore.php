...
class WPRProtectIpstore_V585 {
	...
	function __construct($storage_type = WPRProtectIpstore_V585::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V585::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V585();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V585();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V585::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V585::STORAGE_TYPE_DB) {
