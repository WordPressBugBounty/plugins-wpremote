...
class WPRProtectIpstore_V647 {
	...
	function __construct($storage_type = WPRProtectIpstore_V647::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == WPRProtectIpstore_V647::STORAGE_TYPE_FS) {
			$this->storage = new WPRProtectIpstoreFS_V647();
		} else {
			$this->storage = new WPRProtectIpstoreDB_V647();
		}
	}

	public static function uninstall() {
		WPRProtectIpstoreDB_V647::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == WPRProtectIpstore_V647::STORAGE_TYPE_DB) {
