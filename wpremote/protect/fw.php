...
if (!class_exists('WPRProtectFW_V676')) :
...
class WPRProtectFW_V676 {
	...
	private $mode = WPRProtectFW_V676::MODE_DISABLED;
	private $ip_cookie_mode = WPRProtectFW_V676::IP_COOKIE_MODE_DISABLED;
	private $admin_cookie_mode = WPRProtectFW_V676::ADMIN_COOKIE_MODE_DISABLED;
	private $bypass_level = WPRProtectFW_V676::WP_USER_ROLE_LEVEL_CONTRIBUTOR;
	private $wpf_rule_init_mode = WPRProtectFW_V676::WPF_RULE_INIT_MODE_WP;
	...
	private $rules_mode = WPRProtectFW_V676::RULES_MODE_DISABLED;
	private $is_geo_blocking = false;
	private $is_wp_user_cookie_enabled = false;
	private $log_config = array();
	private $request_profiling_mode = WPRProtectFW_V676::REQ_PROFILING_MODE_DISABLED;
	private $logging_mode = WPRProtectFW_V676::LOGGING_MODE_VISITOR;
	...
	private $log_slice_size = WPRProtectFW_V676::LOG_SLICE_SIZE;
	...
	const DEFAULT_WP_USER_ROLE_LEVELS = array(
		'administrator' => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_ADMIN,
		'editor'        => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_EDITOR,
		'author'        => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_AUTHOR,
		'contributor'   => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_CONTRIBUTOR,
		'subscriber'    => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_SUBSCRIBER
	);

	const EXTRA_WP_USER_ROLE_LEVELS = array(
		'custom'        => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_CUSTOM,
		'unknown'       => WPRProtectFW_V676::WP_USER_ROLE_LEVEL_UNKNOWN
	);
	...
	private function __construct($protect_mode, $request, $config, $brand_name) {
		...
		if ($this->isPrependMode()) {
			...
			$this->ipstore = new WPRProtectIpstore_V676(WPRProtectIpstore_V676::STORAGE_TYPE_FS);
			$this->logger = new WPRProtectLogger_V676($log_file, WPRProtectLogger_V676::TYPE_FS);
		} else {
			$this->ipstore = new WPRProtectIpstore_V676(WPRProtectIpstore_V676::STORAGE_TYPE_DB);
			$this->logger = new WPRProtectLogger_V676(WPRProtectFW_V676::TABLE_NAME, WPRProtectLogger_V676::TYPE_DB);
		}
	...
	public static function getInstance($protect_mode, $request, $config, $brand_name) {
		...
		} elseif (self::$instance->protect_mode != $protect_mode && $protect_mode == WPRProtect_V676::MODE_WP) {
			self::$instance->protect_mode = $protect_mode;
			self::$instance->brand_name = $brand_name;
			self::$instance->ipstore = new WPRProtectIpstore_V676(WPRProtectIpstore_V676::STORAGE_TYPE_DB);
	...
	public static function uninstall() {
		WPRProtect_V676::$db->dropBVTable(WPRProtectFW_V676::TABLE_NAME);
	}
	...
	private function isPrependMode() {
		return ($this->protect_mode === WPRProtect_V676::MODE_PREPEND);
	}

	private function isWPMode() {
		return ($this->protect_mode === WPRProtect_V676::MODE_WP);
	}

	private function isModeDisabled() {
		return ($this->mode === WPRProtectFW_V676::MODE_DISABLED);
	}

	private function isModeProtect() {
		return ($this->mode === WPRProtectFW_V676::MODE_PROTECT);
	}

	private function isAdminCookieEnabled() {
		return ($this->admin_cookie_mode === WPRProtectFW_V676::ADMIN_COOKIE_MODE_ENABLED);
	}

	private function isIPCookieEnabled() {
		return ($this->ip_cookie_mode === WPRProtectFW_V676::IP_COOKIE_MODE_ENABLED);
	}

	private function isRequestProfilingDisabled() {
		return ($this->request_profiling_mode === WPRProtectFW_V676::REQ_PROFILING_MODE_DISABLED);
	}

	private function isRequestProfilingModeDebug() {
		return ($this->request_profiling_mode === WPRProtectFW_V676::REQ_PROFILING_MODE_DEBUG);
	}

	private function isRequestHasValidBypassCookie() {
		if (!isset($this->has_valid_bypass_cookie)) {
			$cookie = (string) $this->request->getCookies(WPRProtectFW_V676::BYPASS_COOKIE_NAME);
	...
	private function isRulesModeProtect() {
		return ($this->rules_mode === WPRProtectFW_V676::RULES_MODE_PROTECT);
	}

	public function isLoggingModeComplete() {
		return ($this->logging_mode === WPRProtectFW_V676::LOGGING_MODE_COMPLETE);
	}

	public function isLoggingModeVisitor() {
		return ($this->logging_mode === WPRProtectFW_V676::LOGGING_MODE_VISITOR);
	}
	...
	private function isWPFRuleInitModePrepend() {
		return ($this->wpf_rule_init_mode === WPRProtectFW_V676::WPF_RULE_INIT_MODE_PREPEND);
	}

	private function isWPFRuleInitModeWP() {
		return ($this->wpf_rule_init_mode === WPRProtectFW_V676::WPF_RULE_INIT_MODE_WP);
	}
	...
	public function setWPUserCookieHandler() {
		if (function_exists('is_user_logged_in') && is_user_logged_in()) {
			...
			if (!$current_wp_user->isIdentical($this->request->wp_user)) {
				$serialized_wp_user = WPRProtectWPUser_V676::_serialize($current_wp_user);
				$cookie_val = $serialized_wp_user . '_' .
					WPRProtectUtils_V676::signMessage($serialized_wp_user, $this->cookie_key);
				$cookie_val = base64_encode($cookie_val);

				$this->setCookie(WPRProtectWPUser_V676::COOKIE_NAME, $cookie_val);
			}
		} elseif ($this->request->wp_user->isLoggedIn()) {
			$this->request->wp_user = WPRProtectWPUser_V676::defaultUser();
			$this->unsetCookie(WPRProtectWPUser_V676::COOKIE_NAME);
		}
	}
	...
	private function getCurrentWPUser() {
		...
		return (new WPRProtectWPUser_V676($id, $role_level, $capabilities, $time));
	}
	...
	private function loadWPUser() {
		...
		$cookie_val = $this->request->getCookies(WPRProtectWPUser_V676::COOKIE_NAME);
		...
		if (WPRProtectUtils_V676::verifyMessage($serialized_user, $signature, $this->cookie_key) === true) {
			$wp_user = WPRProtectWPUser_V676::_unserialize($serialized_user);
			...
			$role_by_level = array_flip(array_merge(WPRProtectFW_V676::DEFAULT_WP_USER_ROLE_LEVELS,
					WPRProtectFW_V676::EXTRA_WP_USER_ROLE_LEVELS));
	...
	private function initRules() {
		...
		if ($this->isPrependMode()) {
			...
			$rule_arrays = WPRProtectUtils_V676::parseFile($rules_file);
		} else {
			$rule_arrays = WPRProtect_V676::$settings->getOption('bvruleset');
		...
		foreach($rule_arrays as $rule_array) {
			$rule = WPRProtectFWRule_V676::init($rule_array);
	...
	private function addWPHook($rule, $hook_name, $function_name, $accepted_args, $hook_type = 'filter') {
		...
		if (empty($this->getWPFRules($function_name))) {
			...
			if ($this->isWPMode()) {
				...
				WPRProtectUtils_V676::preInitWPHook($hook_name, $callback, -9999999, $accepted_args);
			}
		}
	...
	private function setIPCookie() {
		if (!$this->is_ip_cookie_set && $this->isIPCookieEnabled() &&
				!$this->request->getCookies(WPRProtectFW_V676::IP_COOKIE_NAME)) {

			$time = floor(time() / 86400);
			$cookie = hash('sha256', $this->request->ip . $time . $this->cookie_key);
			if ($cookie) {
				$this->setCookie(WPRProtectFW_V676::IP_COOKIE_NAME, $cookie, time() + 86400);
			}
		}
	}
	...
	private function getCurrentWPUserRoleLevel() {
		if (function_exists('current_user_can')) {
			if (function_exists('is_super_admin') &&  is_super_admin()) {
				return WPRProtectFW_V676::WP_USER_ROLE_LEVEL_ADMIN;
			}

			foreach ($this->custom_roles as $role) {
				if (current_user_can($role)) {
					return WPRProtectFW_V676::WP_USER_ROLE_LEVEL_CUSTOM;
				}
			}

			foreach (WPRProtectFW_V676::DEFAULT_WP_USER_ROLE_LEVELS as $role => $level) {
				if (current_user_can($role)) {
					return $level;
				}
			}
	...
	private function getBVCookies() {
		...
		if ($this->request->getCookies(WPRProtectFW_V676::IP_COOKIE_NAME) !== NULL) {
			$cookie_val = (string) $this->request->getCookies(WPRProtectFW_V676::IP_COOKIE_NAME);
			$cookies[WPRProtectFW_V676::IP_COOKIE_NAME] = $cookie_val;
		}
	...
	private function getUserBy($attribute, $value) {
		if (isset($value) && function_exists('get_user_by') && WPRProtectUtils_V676::havePluginsLoaded()) {
		...
		if (is_array($params)) {
			foreach ($params as $key => $value) {
				...
				if (is_array($value)) {
					...
					if ($this->matchCount(WPRProtectFWRule_V676::SQLIREGEX, $value) > 2) {
	...
	private function canRequestBypassFirewall() {
		if ($this->isRequestIPWhitelisted() || $this->isRequestHasValidBypassCookie()) {
			$this->request->category = WPRProtectRequest_V676::CATEGORY_WHITELISTED;
			$this->request->status = WPRProtectRequest_V676::STATUS_BYPASSED;

			return true;
		} elseif (WPRProtectUtils_V676::isPrivateIP($this->request->ip)) {
			$this->request->category = WPRProtectRequest_V676::CATEGORY_PRIVATEIP;
			$this->request->status = WPRProtectRequest_V676::STATUS_BYPASSED;
	...
	private function handleRequestOnRuleMatch($rules, $engine_vars = array(), $log_data = array()) {
		$normalized_engine_vars = WPRProtectFWRuleEngine_V676::normalizeVariables($engine_vars);
		...
		foreach ($rules as $rule) {
			...
			if (array_key_exists('variables', $rule->opts)) {
				$_engine_vars = array_merge($_engine_vars,
					WPRProtectFWRuleEngine_V676::normalizeVariables($rule->opts['variables']));
			}

			$rule_engine = new WPRProtectFWRuleEngine_V676($this->request, $_engine_vars);
			...
			if ($rule_engine->evaluate($rule) && !$rule_engine->hasError()) {
				...
				foreach($rule->actions as $action) {
					...
					case "ALLOW":
						...
						$this->request->category = WPRProtectRequest_V676::CATEGORY_RULE_ALLOWED;
						return;
					case "BLOCK":
						if ($this->isModeProtect()) {
							$this->terminateRequest(WPRProtectRequest_V676::CATEGORY_RULE_BLOCKED);
						}
						return;
					case "INSPECT":
						$this->inspectRequest();
	...
	private function terminateRequest($category) {
		...
		$this->request->status = WPRProtectRequest_V676::STATUS_BLOCKED;
		$this->request->setRespCode(403);

		if ($this->can_set_cache_prevention_cookie &&
			!$this->request->getCookies(WPRProtectFW_V676::PREVENT_CACHE_COOKIE_NAME)) {
			$value = "Prevent Caching Response.";
			$this->setCookie(WPRProtectFW_V676::PREVENT_CACHE_COOKIE_NAME, $value, time() + 43200);
		}
	...
	public function setBypassCookie() {
		if (function_exists('is_user_logged_in') && is_user_logged_in() &&
			...
			if ($role_level >= $this->bypass_level) {
				...
				if ($cookie) {
					$this->setCookie(WPRProtectFW_V676::BYPASS_COOKIE_NAME, $cookie);
				}
			}
		}
	}
