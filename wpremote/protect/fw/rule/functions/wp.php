...
if (!trait_exists('WPRProtectFWRuleWPFunc_V585')) :
trait WPRProtectFWRuleWPFunc_V585 {
	private function _rf_sanitizeUser() {
		...
		if (!function_exists('sanitize_user') || !WPRProtectUtils_V585::haveMupluginsLoaded()) {
	...
	private function _rf_maybeSerialize() {
		...
		if (!function_exists('maybe_serialize') || !WPRProtectUtils_V585::haveMupluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
	...
	private function _rf_isUserLoggedIn() {
		...
		if (!function_exists('is_user_logged_in') || !WPRProtectUtils_V585::havePluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
	...
	private function _rf_getCurrentWPUser() {
		...
		if (!function_exists('wp_get_current_user') || !WPRProtectUtils_V585::havePluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
				$this->addExState("getCurrentWPUser: Func wp_get_current_user doesn't exist.")
			);
		}

		return WPRProtectFWRuleEngine_V585::toAllowedType(wp_get_current_user());
	}
	...
	private function _rf_currentUserCan() {
		...
		if (!function_exists('current_user_can') || !WPRProtectUtils_V585::havePluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
	...
	private function _rf_getUserBy() {
		...
		if (!function_exists('get_user_by') || !WPRProtectUtils_V585::havePluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
		...
		return WPRProtectFWRuleEngine_V585::toAllowedType($user);
	}
	...
	private function _rf_getCurrentWPUserCapabilities() {
		...
		return WPRProtectFWRuleEngine_V585::toAllowedType($user["allcaps"]);
	}
	...
	private function _rf_getOption() {
		...
		if (!function_exists('get_option') || !WPRProtectUtils_V585::haveMupluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
				$this->addExState("getOption: Func get_option doesn't exist.")
			);
		}

		return WPRProtectFWRuleEngine_V585::toAllowedType(get_option($option, $default_value));
	}
	...
	private function _rf_checkPasswordResetKey() {
		...
		if (!function_exists('check_password_reset_key') || !WPRProtectUtils_V585::havePluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
	...
	private function _rf_wpUnslash() {
		...
		if (!function_exists('wp_unslash') || !WPRProtectUtils_V585::haveMuPluginsLoaded()) {
			throw new WPRProtectRuleError_V585(
