...
if (!class_exists('WPRProtect_V665')) :
...
class WPRProtect_V665 {
	...
	public static function init($mode) {
		...
		if ($mode == WPRProtect_V665::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = WPRProtectUtils_V665::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(WPRProtect_V665::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new WPRProtectRequest_V665($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			WPRProtectFW_V665::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			WPRProtectFW_V665::getInstance($mode, $request, $fw_config, $brand_name)->init();
			WPRProtectLP_V665::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		WPRProtectIpstore_V665::uninstall();
		WPRProtectFW_V665::uninstall();
		WPRProtectLP_V665::uninstall();

		WPRProtect_V665::removeWPPrepend();
		WPRProtect_V665::removePHPPrepend();
		WPRProtect_V665::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		WPRProtectUtils_V665::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		WPRProtect_V665::removeHtaccessPrepend();
		WPRProtect_V665::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		WPRProtectUtils_V665::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		WPRProtectUtils_V665::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		WPRProtectUtils_V665::rrmdir($mc_data_dir);
	}
}
endif;