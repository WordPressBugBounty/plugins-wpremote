...
if (!class_exists('WPRProtect_V672')) :
...
class WPRProtect_V672 {
	...
	public static function init($mode) {
		...
		if ($mode == WPRProtect_V672::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = WPRProtectUtils_V672::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(WPRProtect_V672::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new WPRProtectRequest_V672($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			WPRProtectFW_V672::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			WPRProtectFW_V672::getInstance($mode, $request, $fw_config, $brand_name)->init();
			WPRProtectLP_V672::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		WPRProtectIpstore_V672::uninstall();
		WPRProtectFW_V672::uninstall();
		WPRProtectLP_V672::uninstall();

		WPRProtect_V672::removeWPPrepend();
		WPRProtect_V672::removePHPPrepend();
		WPRProtect_V672::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		WPRProtectUtils_V672::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		WPRProtect_V672::removeHtaccessPrepend();
		WPRProtect_V672::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		WPRProtectUtils_V672::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		WPRProtectUtils_V672::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		WPRProtectUtils_V672::rrmdir($mc_data_dir);
	}
}
endif;