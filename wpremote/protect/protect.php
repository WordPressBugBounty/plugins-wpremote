...
if (!class_exists('WPRProtect_V577')) :
...
class WPRProtect_V577 {
	...
	public static function init($mode) {
		...
		if ($mode == WPRProtect_V577::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = WPRProtectUtils_V577::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(WPRProtect_V577::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new WPRProtectRequest_V577($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			WPRProtectFW_V577::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			WPRProtectFW_V577::getInstance($mode, $request, $fw_config, $brand_name)->init();
			WPRProtectLP_V577::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		WPRProtectIpstore_V577::uninstall();
		WPRProtectFW_V577::uninstall();
		WPRProtectLP_V577::uninstall();

		WPRProtect_V577::removeWPPrepend();
		WPRProtect_V577::removePHPPrepend();
		WPRProtect_V577::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		WPRProtectUtils_V577::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		WPRProtect_V577::removeHtaccessPrepend();
		WPRProtect_V577::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		WPRProtectUtils_V577::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		WPRProtectUtils_V577::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		WPRProtectUtils_V577::rrmdir($mc_data_dir);
	}
}
endif;