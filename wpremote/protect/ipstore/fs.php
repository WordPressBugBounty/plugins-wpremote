...
if (!class_exists('WPRProtectIpstoreFS_V636')) :
	class WPRProtectIpstoreFS_V636 {
		...
		function __construct() {
			...
			$ips = WPRProtectUtils_V636::parseFile($ip_store_file);
		...
		public function getTypeIfBlacklistedIP($ip) {
			return $this->getIPType($ip, WPRProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPBlacklisted($ip) {
			return $this->checkIPPresent($ip, WPRProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPWhitelisted($ip) {
			return $this->checkIPPresent($ip, WPRProtectIpstoreFS_V636::IP_TYPE_WHITELISTED);
		}
		...
		private function getIPType($ip, $type) {
			switch ($type) {
			case WPRProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED:
				return isset($this->blacklisted_ips[$ip]) ? $this->blacklisted_ips[$ip] : null;
			case WPRProtectIpstoreFS_V636::IP_TYPE_WHITELISTED:
