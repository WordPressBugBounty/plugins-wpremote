...
if (WPRHelper::getRawParam('REQUEST', 'bvplugname') == "wpremote") {
	...
	if ($bvinfo->hasValidDBVersion()) {
		if ($bvinfo->isProtectModuleEnabled()) {
			...
			WPRProtect_V647::$settings = new WPRWPSettings();
			WPRProtect_V647::$db = new WPRWPDb();
			WPRProtect_V647::$info = new WPRInfo(WPRProtect_V647::$settings);

			add_action('wpr_clear_pt_config', array('WPRProtect_V647', 'uninstall'));

			if ($bvinfo->isActivePlugin()) {
				WPRProtect_V647::init(WPRProtect_V647::MODE_WP);
			}
		}
